...
### UC-03: تعريف سمات ديناميكية وربطها بفئات/منتجات

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات |
| **المتطلبات المسبقة** | تسجيل دخول بصلاحية إدارة السمات |
| **التدفق الأساسي** | 1. تعريف AttributeDefinition (نوع/تسمية/تحقق) ← 2. إنشاء AttributeSet وربط السمات ← 3. ربط المجموعة بالفئة أو المنتج ← 4. تعبئة القيم في attribute_value |
| **التدفقات البديلة** | فشل التحقق (JSON Schema/Regex): رفض مع رسالة خطأ · سمة واجبة فارغة: منع الحفظ |
| **النتيجة** | السمات محفوظة ومفهرسة وقابلة للبحث |
| **المتطلبات** | FR-020, FR-021, FR-022 |

### UC-04: إعداد وحدات القياس والتحويل

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات |
| **المتطلبات المسبقة** | تسجيل دخول بصلاحية إدارة المنتجات |
| **التدفق الأساسي** | 1. تعريف وحدة قياس جديدة (UoM) ← 2. تعريف معاملات التحويل بين الوحدات ← 3. ربط الوحدات بالمنتج (أساسية/بديلة) ← 4. تحديد الحد الأدنى/الأقصى للكمية |
| **التدفقات البديلة** | تحويل دائري: رفض · معامل تحويل ≤ 0: رفض |
| **النتيجة** | وحدات القياس محفوظة مع التحويلات |
| **المتطلبات** | FR-030 |

### UC-05: تكوين منتج مركّب (BOM/Bundle)

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات |
| **المتطلبات المسبقة** | وجود منتجات مكوّنة نشطة |
| **التدفق الأساسي** | 1. إنشاء منتج أب (Bundle/KIT) ← 2. إضافة المكوّنات مع الكميات ← 3. تحديد سياسة التفجير (EXPLODE/NO_EXPLODE) ← 4. تحديد نسب توزيع السعر ← 5. حفظ التركيب |
| **التدفقات البديلة** | ربط منتج بنفسه: رفض · مكوّن غير نشط: تحذير · مرجع دائري: رفض |
| **النتيجة** | منتج مركّب مع مكوّناته ونسب التوزيع |
| **المتطلبات** | FR-031, FR-032, FR-040 |

### UC-07: تفعيل/تعطيل قنوات المنتج

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات |
| **المتطلبات المسبقة** | وجود منتج نشط + قنوات معرّفة |
| **التدفق الأساسي** | 1. اختيار المنتج ← 2. تمكين/تعطيل القنوات المطلوبة ← 3. تكوين صيغة العرض والحدود لكل قناة ← 4. تكوين Feature Flags ← 5. حفظ |
| **التدفقات البديلة** | تفعيل قناة بدون تسعير فعّال: رفض (BR-02) |
| **النتيجة** | القنوات مفعّلة مع إعداداتها |
| **المتطلبات** | FR-050, FR-051, FR-052 |

### UC-08: إعداد الترقيم المتعدد

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات |
| **المتطلبات المسبقة** | تسجيل دخول بصلاحية إدارة الترقيم |
| **التدفق الأساسي** | 1. إنشاء NumberingScheme مع نمط المقاطع ← 2. تكوين سياسة الفجوات ← 3. إنشاء مخازن تسلسل للفروع/القنوات ← 4. ربط المخطط بالمنتج/النوع ← 5. معاينة الترقيم المتوقع |
| **التدفقات البديلة** | نمط ترقيم غير صالح: رفض · تعارض في حجز الرقم: إعادة محاولة تلقائية |
| **النتيجة** | مخطط ترقيم جاهز مع مخازن تسلسل |
| **المتطلبات** | FR-070, FR-071, FR-072, FR-073, FR-074 |

### UC-09: ربط خرائط المحاسبة

| | |
|---|---|
| **الفاعل الرئيسي** | المالية |
| **المتطلبات المسبقة** | وجود قوالب قيود محاسبية + منتج نشط |
| **التدفق الأساسي** | 1. اختيار المنتج ← 2. ربط قالب قيد لكل حدث (بيع/مرتجع/صرف/فائدة/رسم/تأخير) ← 3. معاينة القيود المتوقعة ← 4. حفظ الربط |
| **التدفقات البديلة** | حدث بدون قالب: تحذير عند التفعيل · قالب غير متوازن (مدين ≠ دائن): رفض |
| **النتيجة** | المنتج مرتبط بقوالب محاسبية لجميع الأحداث |
| **المتطلبات** | FR-080, FR-081, FR-082 |

### UC-10: تعريف الرسوم والغرامات

| | |
|---|---|
| **الفاعل الرئيسي** | مدير المنتجات / المالية |
| **المتطلبات المسبقة** | تسجيل دخول بصلاحية إدارة الرسوم |
| **التدفق الأساسي** | 1. تعريف Charge (نوع/أساس/قيمة/توقيت) ← 2. ربط الرسم بالمنتج ← 3. تحديد معاملات التجاوز إن وُجدت ← 4. معاينة الرسوم المحسوبة |
| **التدفقات البديلة** | قيمة سالبة: رفض · نوع غير معروف: رفض |
| **النتيجة** | الرسوم مرتبطة بالمنتج وجاهزة للاحتساب |
| **المتطلبات** | FR-100, FR-101, FR-102 |

### UC-11: إعداد الأهلية والوثائق والضمانات

| | |
|---|---|
| **الفاعل الرئيسي** | المخاطر / المالية |
| **المتطلبات المسبقة** | وجود منتج مالي |
| **التدفق الأساسي** | 1. تعريف قواعد أهلية CEL (Score/DSR/KYC) ← 2. تحديد الوثائق المطلوبة (إلزامية/اختيارية) ← 3. تحديد متطلبات الضمان ونسب التغطية ← 4. ربط الكل بالمنتج |
| **التدفقات البديلة** | تعبير CEL غير صالح: رفض · نسبة تغطية ≤ 0: رفض |
| **النتيجة** | شروط الأهلية والوثائق والضمانات مرتبطة بالمنتج |
| **المتطلبات** | FR-090, FR-091, FR-092 |

### UC-14: مطالبات وفوترة الأقساط

| | |
|---|---|
| **الفاعل الرئيسي** | المالية / النظام (آلي) |
| **المتطلبات المسبقة** | عقد نشط مع جدول أقساط |
| **التدفق الأساسي** | 1. توليد مطالبة عند حلول موعد القسط ← 2. إرسال إشعار للعميل ← 3. تسجيل الدفعة عند الاستلام ← 4. تحديث حالة القسط (PAID/PARTIAL) ← 5. تسجيل قيد في Sub-ledger |
| **التدفقات البديلة** | عدم الدفع خلال مهلة Grace: تطبيق غرامة (BR-05) · تأخر 30+ يوم: تصعيد حسب Aging (BR-08) · طلب تأجيل: تعديل الجدول وفق القواعد |
| **النتيجة** | المطالبة مُعالجة والقيود مسجلة |
| **المتطلبات** | FR-110, FR-111, FR-112, FR-134 |

### UC-15: تدقيق وتتبّع الحالات

| | |
|---|---|
| **الفاعل الرئيسي** | الدعم / التدقيق |
| **المتطلبات المسبقة** | تسجيل دخول بصلاحية التدقيق |
| **التدفق الأساسي** | 1. البحث عن كيان (منتج/عقد/حجز) ← 2. عرض سجل التغييرات (audit_log) ← 3. عرض انتقالات الحالة (state_transition) ← 4. مقارنة إصدارين (Diff View) ← 5. عرض الأحداث (domain_event) |
| **التدفقات البديلة** | تصدير السجل إلى CSV/PDF · تصفية حسب الفترة/المستخدم/نوع الإجراء |
| **النتيجة** | سجل تدقيق كامل مع إمكانية المقارنة |
| **المتطلبات** | FR-140, FR-141, FR-142 |
